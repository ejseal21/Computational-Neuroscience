...
class MotionNet:
    ...
    def __init__(self, dt, n_dirs, lvl1_params, lv1_hgate_params, lvl2_inter_params=None,
                 lvl2_params=None, lvl3_params=None, lvl4_params=None, lvl4_33_params=None, lvl4_67_params=None, lvl5_params=None,
                 lvl6_params=None,
                 lvl3_excit_ker_params=None, lvl4_excit_ker_params=None, lvl4_inhib_ker_params=None,
                 lvl4_33_excit_ker_params=None, lvl4_67_excit_ker_params=None, lvl4_67_inhib_ker_params=None,
                 lvl5_excit_ker_params=None, lvl6_inhib_ker_params=None,
                 do_lvl1=True, do_lvl2=True, do_lvl3=True, do_lvl4=True, do_lvl4_33=True, do_lvl4_67=True, do_lvl5=True, do_lvl6=True):
        '''
        Parameters:
        -----------
        dt. float.
        ...
        self.mrf_excit_ker = None
        self.comp2_excit_ker = None
        self.comp2_inhib_ker = None
        ...
        self.do_lvl4_33 = do_lvl4_33
        self.do_lvl4_67 = do_lvl4_67
        ...
        self.layer4_33 = lvl4_33_params
        self.layer4_33_excite = lvl4_33_excit_ker_params
        self.layer4_67 = lvl4_67_params
        self.layer4_67_excite = lvl4_67_excit_ker_params
        self.layer4_67_inhib = lvl4_67_inhib_ker_params
    ...
    def make_kernels(self):
        ...
        #layer 4.33
        if self.do_lvl4_33:
            self.mrf_excit_ker = np.zeros((self.n_dirs, self.layer4_33_excite.get_size()[0], self.layer4_33_excite.get_size()[1]))
            for i in range(self.n_dirs):
                self.mrf_excit_ker[i] = filters.aniso_gauss(i, sigmas=self.layer4_33_excite.get_sigma(), sz=self.layer4_33_excite.get_size())
        #layer 4.67
        if self.do_lvl4_67:
            self.comp2_excit_ker = np.zeros((self.n_dirs, self.layer4_67_excite.get_size()[0], self.layer4_67_excite.get_size()[1]))
            for i in range(self.n_dirs):
                self.comp2_excit_ker[i] = filters.aniso_gauss(i, sigmas=self.layer4_67_excite.get_sigma(), sz=self.layer4_67_excite.get_size())
            self.comp2_inhib_ker = filters.iso_gauss(sigma=self.layer4_67_inhib.get_sigma(), sz=self.layer4_67_inhib.get_size())
        #layer 5
        if self.do_lvl5:
        ...
        #layer 6
        if self.do_lvl6:
            self.mstd_inhib_ker = filters.iso_gauss(sigma=self.layer6_inhib.get_sigma(), sz = self.layer6_inhib.get_size())
    ...
    def initialize(self, n_steps, height, width):
        ...
        *Layer activity placeholders* and *Layer output activity placeholders*).
        ...
        #layer 4.33
        self.mrf_cells = np.zeros((n_steps, self.n_dirs, height, width))
        self.mrf_out = np.zeros((n_steps, self.n_dirs, height, width))

        #layer 4.67
        # #layer 4
        self.comp2_cells = np.zeros((n_steps, self.n_dirs, height, width))
        self.comp2_out = np.zeros((n_steps, self.n_dirs, height, width))
    ...
    def d_mid_range(self, t):
        d_mrf = np.zeros((self.n_dirs, self.height, self.width))
        for k in range(self.n_dirs):
            d_mrf[k] = self.layer4_33.get_time_const() * (- np.expand_dims(self.mrf_cells[t-1, k], 0) + signal.convolve(self.comp_out[t, k], self.mrf_excit_ker[k], "same"))
        return d_mrf

    def d_competition_layer2(self, t):
        d_comp = np.zeros((self.n_dirs, self.height, self.width))
        for k in range(self.n_dirs):
            opposite = self.get_opponent_direction(k)
            d_comp[k] = self.layer4_67.get_time_const() * (- np.expand_dims(self.comp2_cells[t-1, k], 0) + (1-self.comp2_cells[t-1, k]) * signal.convolve(self.mrf_out[t, k], self.comp2_excit_ker[k], "same") - (self.comp2_cells[t-1, k] + self.layer4_67.get_lower_bound()) * (signal.convolve(self.mrf_out[t, k], self.comp2_inhib_ker, "same") + self.mrf_out[t, opposite]))
    ...
    def d_long_range(self, t):
        ...
        for k in range(self.n_dirs):
            if self.do_lvl4_67:
                conv = self.comp2_out[t,k]
            elif self.do_lvl4_33:
                conv = self.mrf_out[t,k]
            else:
                conv = self.comp_out[t,k]
            if self.do_lvl6 == False:
                fb = 0
            else:
                fb = self.mstd_fb(t,self.mstd_out[t-1])[k]
            d_lrf[k] = self.layer5.get_time_const() * (- np.expand_dims(self.lr_cells[t-1, k], 0) + (1 - self.lr_cells[t-1, k]) * signal.convolve(conv, self.long_range_excit_ker[k], "same") - (self.lr_cells[t-1, k] + self.layer5.get_lower_bound()) * fb) 
        return d_lrf
    ...
    def update_net(self, t):
        ...
        #layer 4.33
        if self.do_lvl4_33:
            d_mr = self.d_mid_range(t)
            self.mrf_cells[t] = self.mrf_cells[t-1] + d_mr * self.dt
            self.mrf_out[t] = np.maximum(self.mrf_cells[t] - self.layer4_33.get_output_thres(), 0)
        
        #layer 4.67
        if self.do_lvl4_67:
            d_comp2 = self.d_competition_layer2(t)
            self.comp2_cells[t] = self.comp2_cells[t-1] + d_comp2 * self.dt
            self.comp2_out[t] = np.maximum(self.comp2_cells[t] - self.layer4_67.get_output_thres(), 0)
